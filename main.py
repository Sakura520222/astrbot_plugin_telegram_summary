...
class TelegramSummaryPlugin(Star):
    ...
    def _validate_channels(self, channels) -> list:
        ...
        for channel in channels:
            if not isinstance(channel, str):
                logger.warning(
                    f"跳过非字符串频道配置: {channel} (类型: {type(channel).__name__})"
                )
                continue
    ...
    def _validate_ai_provider(self, provider):
        ...
        if not isinstance(provider, str) or not provider.strip():
            raise ValueError(f"AI 提供商配置必须是非空字符串，当前值: {provider}")
    ...
    def _init_concurrent_safety(self):
        ...
        self._telegram_client_lock = (
            asyncio.Lock()
        )  # 新增：防止 Telegram Client 并发冲突
    ...
    def _setup_scheduler(self):
        ...
        self.scheduler.add_job(
            self.main_job, "cron", day_of_week=day_of_week, hour=hour, minute=minute
        )
    ...
    def _match_channel(self, user_input: str, config_channel: str) -> bool:
        ...
        return (
            user_input in config_identifiers  # 直接匹配
            or user_channel_name in config_identifiers  # 提取后的频道名匹配
        )
    ...
    async def _cleanup_login_session(self, sender_id: str):
        ...
        Args:
            sender_id: 用户ID
        """
        if sender_id in self.login_states and self.login_states[sender_id].get(
            "client"
        ):
            try:
    ...
    async def _handle_phone_stage(
        self, event, user_input: str, login_state: dict, sender_id: str
    ):
        """处理登录流程的手机号输入阶段
        ...
        # 提示正在连接
        await event.send(
            event.plain_result("📡 正在连接到 Telegram 服务器并请求验证码...")
        )
        ...
        async with self._telegram_client_lock:
            try:
                ...
                logger.info(
                    f"为用户 {sender_id} 创建Telegram客户端，会话文件: {session_file}"
                )
    ...
    def load_prompt(self):
        ...
        try:
            ...
            logger.warning(
                f"提示词文件 {self.PROMPT_FILE} 不存在，将使用默认提示词并创建文件"
            )
            ...
            logger.error(
                f"读取提示词文件 {self.PROMPT_FILE} 时出错: {type(e).__name__}: {e}"
            )
            # 如果读取失败，使用默认提示词
            return self.DEFAULT_PROMPT
    ...
    def save_prompt(self, prompt):
        ...
        try:
            ...
            logger.error(
                f"保存提示词到文件 {self.PROMPT_FILE} 时出错: {type(e).__name__}: {e}"
            )
    ...
    def load_config(self):
        ...
        try:
            ...
            logger.error(
                f"读取配置文件 {self.CONFIG_FILE} 时出错: {type(e).__name__}: {e}"
            )
            return {}
    ...
    def save_config(self, config):
        ...
        try:
            ...
            logger.error(
                f"保存配置到文件 {self.CONFIG_FILE} 时出错: {type(e).__name__}: {e}"
            )
    ...
    def load_last_summary_times(self):
        ...
        try:
            with open(self.LAST_SUMMARY_FILE, encoding="utf-8") as f:
                data = json.load(f)
                last_times = {}
                for channel, time_str in data.items():
                    if time_str:
                        ...
                        last_time = datetime.fromisoformat(time_str).replace(
                            tzinfo=timezone.utc
                        )
            ...
            logger.warning(
                f"上次总结时间文件 {self.LAST_SUMMARY_FILE} 不存在，将使用默认值"
            )
            return {}
        except json.JSONDecodeError as e:
            ...
            logger.error(
                f"读取上次总结时间文件 {self.LAST_SUMMARY_FILE} 时出错: {type(e).__name__}: {e}"
            )
            return {}
    ...
    def save_last_summary_times(self, times):
        ...
        try:
            ...
            logger.error(
                f"保存各频道上次总结时间到文件 {self.LAST_SUMMARY_FILE} 时出错: {type(e).__name__}: {e}"
            )
        ...
        async with self._telegram_client_lock:
            ...
            try:
                async with TelegramClient(
                    self.USER_SESSION_FILE, int(self.api_id), self.api_hash
                ) as client:
                    ...
                    for channel in channels:
                        ...
                        try:
                            # 为每个频道确定独立的起始时间
                            if (
                                channel in self.last_summary_times
                                and self.last_summary_times[channel]
                            ):
                                start_time = self.last_summary_times[channel]
                                logger.info(
                                    f"频道 {channel} 使用上次总结时间作为起始时间: {start_time}"
                                )
                            else:
                                start_time = current_time - timedelta(
                                    days=self.DEFAULT_SUMMARY_DAYS
                                )
                                logger.info(
                                    f"频道 {channel} 没有上次总结时间，使用默认时间范围: 过去{self.DEFAULT_SUMMARY_DAYS}天 ({start_time})"
                                )
                            ...
                            async for message in client.iter_messages(
                                ...
                                if message.text:
                                    ...
                                    channel_messages.append(
                                        f"内容: {message.text[: self.MESSAGE_TRUNCATE_LENGTH]}\n链接: {msg_link}"
                                    )
                            ...
                            logger.error(
                                f"抓取频道 {channel} 时出错: {type(channel_error).__name__}: {channel_error}"
                            )
                            # 继续处理其他频道，不中断整个流程
                            channel_messages = []
                        ...
                        logger.info(
                            f"频道 {channel} 抓取完成，共处理 {channel_message_count} 条消息，其中 {len(channel_messages)} 条包含文本内容"
                        )

                    logger.info(
                        f"所有指定频道消息抓取完成，共处理 {total_message_count} 条消息"
                    )
                    return messages_by_channel

            except Exception as e:
                logger.error(f"Telegram客户端连接失败: {type(e).__name__}: {e}")
                raise Exception("无法连接到Telegram: 请检查网络连接和登录状态") from e
    ...
    async def analyze_with_ai(self, messages):
        ...
        logger.debug(
            f"AI请求配置: 提供商={self.ai_provider}, 提示词长度={len(self.current_prompt)}字符, 上下文长度={len(context_text)}字符"
        )
    ...
    async def _send_admin_alert(
        self, task_name: str, error: Exception, context: dict = None
    ):
        """向管理员发送告警消息
        ...
        try:
            ...
            logger.info(
                f"已向管理员 {self.admin_id} 发送告警: {task_name} - {error_type}"
            )
    ...
    async def push_summary_to_targets(self, summary_text, channel_name):
        ...
        # 使用配置的消息模板构建推送消息
        title_template = self.message_templates.get(
            "summary_title", "【频道周报】{channel_name}"
        )
    ...
    async def main_job(self):
        ...
        if not os.path.exists(self.USER_SESSION_FILE):
            logger.warning(
                f"用户会话文件不存在: {self.USER_SESSION_FILE}，跳过本次自动总结任务"
            )
            logger.info(
                "请管理员使用 /tg_login 命令完成首次登录，之后将正常执行自动总结"
            )
            return
        ...
        try:
            ...
            for channel, messages in messages_by_channel.items():
                ...
                if not summary or summary.startswith("AI 分析失败"):
                    ...
                    total_push_fail += len(self.auto_push_groups) + len(
                        self.auto_push_users
                    )
            ...
            # 输出统计日志
            logger.info(
                f"【自动推送】总结完成。处理频道: {total_channels} 个，"
                f"无消息频道: {empty_channels} 个，"
                f"已推送至 {total_push_success} 个目标（群组和用户）。失败: {total_push_fail}。"
            )
            logger.info(
                f"定时任务完成: {end_time}，总处理时间: {processing_time:.2f}秒"
            )
        except Exception as e:
            end_time = datetime.now(timezone.utc)
            processing_time = (end_time - start_time).total_seconds()
            logger.error(
                f"定时任务执行失败: {type(e).__name__}: {e}，开始时间: {start_time}，结束时间: {end_time}，处理时间: {processing_time:.2f}秒"
            )
    ...
    async def handle_manual_summary(self, event: AstrMessageEvent):
        ...
        if not os.path.exists(self.USER_SESSION_FILE):
            logger.info(
                f"用户会话文件不存在: {self.USER_SESSION_FILE}，自动进入登录流程"
            )
            yield event.plain_result(
                "⚠️ **未检测到登录信息**\n\n"
        ...
        try:
            ...
            # 保存所有频道的上次总结时间
            self.save_last_summary_times(self.last_summary_times)
            logger.info("已保存各频道的上次总结时间")

            logger.info(f"命令 {command} 执行成功")
        except Exception as e:
            logger.error(
                f"执行命令 {command} 时出错: {type(e).__name__}: {e}", exc_info=True
            )
    ...
    async def handle_set_prompt(self, event: AstrMessageEvent):
        ...
        yield event.plain_result(
            f"请发送新的提示词，我将使用它来生成总结。\n\n当前提示词：\n{self.current_prompt}"
        )

        @session_waiter(timeout=60, record_history_chains=False)
        async def setprompt_session(
            ...
            logger.info(
                f"用户 {sender_id} 提交了新的提示词，长度: {len(new_prompt)} 字符"
            )
    ...
    async def handle_add_channel(self, event: AstrMessageEvent):
        ...
        try:
            ...
            yield event.plain_result(
                f"频道 {channel_url} 已成功添加到列表中\n\n当前频道数量：{len(self.channels)}"
            )

        except ValueError:
            # 没有提供频道URL
            yield event.plain_result(
                "请提供有效的频道URL，例如：/addchannel https://t.me/examplechannel"
            )
        except Exception as e:
    ...
    async def handle_delete_channel(self, event: AstrMessageEvent):
        ...
        try:
            ...
            yield event.plain_result(
                f"频道 {channel_url} 已成功从列表中删除\n\n当前频道数量：{len(self.channels)}"
            )

        except ValueError:
            # 没有提供频道URL或频道不存在
            yield event.plain_result(
                "请提供有效的频道URL，例如：/deletechannel https://t.me/examplechannel"
            )
        except Exception as e:
    ...
    async def handle_clear_summary_time(self, event: AstrMessageEvent):
        ...
        try:
            ...
            logger.error(
                f"清除上次总结时间时出错: {type(e).__name__}: {e}", exc_info=True
            )
    ...
    async def handle_tg_login(self, event: AstrMessageEvent):
        ...
        async with self._login_states_lock:
            ...
            if sender_id in self.login_states:
                yield event.plain_result(
                    "您已经在登录流程中，请先完成或使用 /tg_login 重新开始"
                )
                return
        ...
        async def tg_login_session(
            ...
            if sender_id not in self.login_states:
                await event.send(
                    event.plain_result("登录会话已过期，请使用 `/tg_login` 重新开始")
                )
                controller.stop()
                return
            ...
            match stage:
                case "phone":
                    success, should_stop = await self._handle_phone_stage(
                        event, user_input, login_state, sender_id
                    )
                    if not should_stop:
                        controller.keep(timeout=120, reset_timeout=True)

                case "code":
                    success, should_stop = await self._handle_code_stage(
                        event, user_input, login_state, sender_id
                    )
                    if not should_stop:
                        controller.keep(timeout=120, reset_timeout=True)
                ...
                case "password":
                    await self._handle_password_stage(
                        event, user_input, login_state, sender_id
                    )
                ...
                case _:
                    ...
                    await event.send(
                        event.plain_result("登录状态异常，请使用 `/tg_login` 重新开始")
                    )
                    await self._cleanup_login_session(sender_id)
                    controller.stop()
